# Interactive Visualization for "Quasi-Symmetric Nets: A Constructive Approach to the Equimodular Elliptic Type of Kokotsakis Polyhedra" (Example 2)
...
def convert_angles_to_vertices(Alphas, Gammas, Deltas, DihedralAngles, Edge):
    ...
    a2 = generate_edge(a1, delta1, delta2, delta3, booster=0.7)  # Edge length A2A3
    ...
    alpha3_tilde = generate_tilde_angle(alpha1, alpha2, booster=0.1)  # Angle A2B2B1
    alpha4_tilde = 360.0 - alpha1 - alpha2 - alpha3_tilde  # Angle A1B1B2
    

    # Edge lengths for Plane 1
    A2B2 = generate_edge(a1, alpha1, alpha2, alpha3_tilde, booster=1) 
    ...
    A2C2 = generate_edge(a2, gamma3, gamma2, gamma1_tilde, booster=2)
    ...
    alpha1_tilde = generate_tilde_angle(alpha4, alpha3, booster=0.1)  # Angle A3B3B4
    alpha2_tilde = 360.0 - alpha3 - alpha4 - alpha1_tilde  # Angle A4B4B3

    # Edge lengths for Plane 3
    A3B3 = generate_edge(a4, alpha4, alpha3, alpha1_tilde, booster=1.4)
    ...
    A4C4 = generate_edge(a3, gamma1, gamma4, gamma2_tilde, booster=1.4)
...
def visualize_here(Alphas, Betas, Gammas, Deltas, Edge):
    ...
    def update(val):
        nonlocal Vertices, vertices_list, AllEdgesToDraw, labels_list, angle_label_list, edge_label_list, psi2, psi1_init, psi2_init, theta_init, DihedralAngles
        # Update dihedral angle phi from slider
        # phi = phi_slider.val
        
        psi2 = psi2_slider.val

        tilde_e = -1
        
        if psi2 != 0 and psi2 != 360:
            W2 = 1 / tan(psi2 / 2)                    
            try:
                with np.errstate(invalid='raise', divide='raise'):
                    
                    Z =  (2 * np.sqrt(6) * W2 + tilde_e * np.sqrt(2 * np.sqrt(3)) * np.sqrt(2 + 2 * np.sqrt(3) * W2**2 + W2**4)) / (-2 + (np.sqrt(3) + 1) * W2**2)

                    W1 = -tilde_e * np.sqrt(2 * np.sqrt(3)) * np.sqrt(2 + 2 * np.sqrt(3) * W2**2 + W2**4) / (2 + (np.sqrt(3) - 1) * W2**2)

                    U = (-2 * np.sqrt(6) * W2 + tilde_e * np.sqrt(2 * np.sqrt(3)) * np.sqrt(2 + 2 * np.sqrt(3) * W2**2 + W2**4)) / (-2 + (np.sqrt(3) + 1) * W2**2)


            except (FloatingPointError, ZeroDivisionError, ValueError):
                Z, W1, U = None, None, None
        else: 
            print("Dihedral Angle should be between 0 and 360 degrees.")
            return 
        
        if None not in [Z, W1, W2] and all(np.isfinite([Z, W1, W2])):
            # Angles
            psi1_angle = 2 * arctan(1 / W1)
            phi_angle = 2 * arctan(1 / Z)
    ...
    ax_psi2 = plt.axes([0.15, 0.05, 0.7, 0.02])  # Slider for psi2

    # Create the slider to control the dihedral angle psi2
    psi2_slider = Slider(
        ax=ax_psi2,
        label=r'Angle $\psi_2$ (degrees)',
        valmin=0.0,
        valmax=360.0,
        valinit=psi2,
        valstep=0.01,
    ...
    ax_button_minus = plt.axes([0.15, 0.01, 0.03, 0.02])  # Button to decrement psi2
    ax_button_plus = plt.axes([0.81, 0.01, 0.03, 0.02])   # Button to increment psi2
    ...
    def decrement_psi2(event):
        new_psi2 = max(psi2_slider.val - 0.01, psi2_slider.valmin)
        psi2_slider.set_val(new_psi2)

    def increment_psi2(event):
        new_psi2 = min(psi2_slider.val + 0.01, psi2_slider.valmax)
        psi2_slider.set_val(new_psi2)
    ...
    button_minus.on_clicked(decrement_psi2)
    button_plus.on_clicked(increment_psi2)
...
def main():
    ...
    def ZZ(W2):
        numerator = (2 * np.sqrt(6) * W2 + ttilde_e * np.sqrt(2 * np.sqrt(3)) * np.sqrt(2 + 2 * np.sqrt(3) * W2**2 + W2**4))
        denominator = (-2 + (np.sqrt(3) + 1) * W2**2)
        return numerator / denominator
    
    def WW1(W2):
        return -ttilde_e * np.sqrt(2 * np.sqrt(3)) * np.sqrt(2 + 2 * np.sqrt(3) * W2**2 + W2**4) / (2 + (np.sqrt(3) - 1) * W2**2)
    ...
    WW2_vals = 0.1
    WW1_vals = WW1(WW2_vals)
    ZZ_vals = ZZ(WW2_vals)
    UU_vals = UU(WW2_vals)
    
    DihedralAngles = [2 * np.degrees(np.arctan(1/ZZ_vals)), 2 * np.degrees(np.arctan(1/WW2_vals)), 2 * np.degrees(np.arctan(1/UU_vals)), 2 * np.degrees(np.arctan(1/WW1_vals))]
    
    #NON-FLEXIBLE EXAMPLE
    # # ZZ_vals = np.sqrt(3 * (1 + np.sqrt(3)) / 2)
    # WW2_vals = np.sqrt(1 + np.sqrt(3))
    # # UU_vals = -np.sqrt(3 * (1 + np.sqrt(3)) / 2)
    # WW1_vals = np.sqrt(3*(1 + np.sqrt(3))/2)
    
    # DihedralAngles = [180, 2 * np.degrees(np.arctan(1/WW2_vals)), 180, 2 * np.degrees(np.arctan(1/WW1_vals))] 
    ...
    # create_obj_file_of_vertices_and_faces(Vertices, 1)
    # Vertices = convert_angles_to_vertices(Alphas, Gammas, Deltas, DihedralAngles_Init, Edge)
    # create_obj_file_of_vertices_and_faces(Vertices, 2)
