# Interactive Visualization for "Quasi-Symmetric Nets: A Constructive Approach to the Equimodular Elliptic Type of Kokotsakis Polyhedra" (Example 1)
        ...
        if psi2 != 0 and psi2 != 360:
            ...
            try:
                with np.errstate(invalid='raise', divide='raise'):
                    ...
                    Z = (np.sqrt(2) * W2 - np.sqrt(6) * W2 + tilde_e * np.sqrt(2 + 6 * W2**2 + 3 * W2**4)) / (1 + np.sqrt(3) + 3 * W2**2)

                    W1 = tilde_e * np.sqrt(2 + 6 * W2**2 + 3 * W2**4) / (np.sqrt(3) + 1 + np.sqrt(3) * W2**2)

                    U = (np.sqrt(2) * (-1 + np.sqrt(3)) * W2 +  tilde_e * np.sqrt(2 + 6 * W2**2 + 3 * W2**4)) / (1 + np.sqrt(3) + 3 * W2**2)
...
def main():
    ...
    def ZZ(W2):
        numerator = np.sqrt(2) * W2 - np.sqrt(6) * W2 + ttilde_e * np.sqrt(2 + 6 * W2**2 + 3 * W2**4)
        denominator = 1 + np.sqrt(3) + 3 * W2**2
        return numerator / denominator
    
    def WW1(W2):
        return ttilde_e * np.sqrt(2 + 6 * W2**2 + 3 * W2**4) / (np.sqrt(3) + 1 + np.sqrt(3) * W2**2)
    
    def UU(W2):
        numerator = np.sqrt(2) * (-1 + np.sqrt(3)) * W2 + ttilde_e * np.sqrt(2 + 6 * W2**2 + 3 * W2**4)
        denominator = 1 + np.sqrt(3) + 3 * W2**2
        return numerator / denominator
