# Interactive Visualization for "Quasi-Symmetric Nets: A Constructive Approach to the Equimodular Elliptic Type of Kokotsakis Polyhedra" (Example 4)
...
def generate_edge(A1, angle1, angle2, angle3, booster=1.0):
    ...
    if sum_angle2_3 >= 180.0:
        ...
        if sum_angle1_2 >= 180.0:
            ...
            if booster == 1000.0:
                print("Choose A2 as you wish. For example, A2 = A1.")
        else:
...
def convert_angles_to_vertices(Alphas, Gammas, Deltas, DihedralAngles, Edge):
    ...
    a2 = generate_edge(a1, delta1, delta2, delta3, booster=4)  # Edge length A2A3
    ...
    A2B2 = generate_edge(a1, alpha1, alpha2, alpha3_tilde, booster=0.6) 
    ...
    gamma1_tilde = generate_tilde_angle(gamma3, gamma2, booster=0.8)  # Angle A2C2C3
    gamma4_tilde = 360.0 - gamma2 - gamma3 - gamma1_tilde  # Angle A3C3C2
    

    # Edge lengths for Plane 2
    A2C2 = generate_edge(a2, gamma3, gamma2, gamma1_tilde, booster=0.5)
    ...
    alpha1_tilde = generate_tilde_angle(alpha4, alpha3, booster=0.8)  # Angle A3B3B4
    ...
    A4C4 = generate_edge(a3, gamma1, gamma4, gamma2_tilde, booster=0.6)
        ...
        if phi != 0 and phi != 360:
            ...
            try:
                with np.errstate(invalid='raise', divide='raise'):
                    W2 = (1.3011995890502461 * (
                5.832687748284718 * Z
                - tilde_e * 1.6606733509905591 * np.sqrt((1 - 0.5721009760467486 * Z**2) *
                                               (-1 + 7.594436687028424 * Z**2))
            )) / (3.588495042139688 + 7.594436687028424 * Z**2)

                    W1 = (2.2689737907253456 * (
                10.50393110877767 * Z
                - tilde_e * 2.542115267025096 * np.sqrt((1 - 0.5721009760467486 * Z**2) *
                                              (-1 + 7.594436687028424 * Z**2))
            )) / (14.662902846473798 + 7.594436687028424 * Z**2)
                    
                    U = (0.20405148639750417 * (
                1.1778865025892307 * Z
                + tilde_e * 10.102197432038 * np.sqrt((1 - 0.5721009760467486 * Z**2) *
                                            (-1 + 7.594436687028424 * Z**2))
            )) / (3.6622434357916323 - 2.099237700754583 * Z**2)
...
def main():
    ...
    Alphas = [91.32487959870187, 115.75047063536742, 31.19200181228523, 28.19551791700786]
    Betas = [27.53122212644406, 29.335366103921295, 113.66642350596918, 107.67391515450669]
    Gammas = [103.21844931187813, 109.7807695499394, 89.61760260813426, 121.75654544610931]
    Deltas = [120, 80, 85, 75]
    ...
    def WW1(Z):
        return (2.2689737907253456 * (
                10.50393110877767 * Z
                - ttilde_e * 2.542115267025096 * np.sqrt((1 - 0.5721009760467486 * Z**2) *
                                              (-1 + 7.594436687028424 * Z**2))
            )) / (14.662902846473798 + 7.594436687028424 * Z**2)

    def WW2(Z):
        return (1.3011995890502461 * (
                5.832687748284718 * Z
                - ttilde_e * 1.6606733509905591 * np.sqrt((1 - 0.5721009760467486 * Z**2) *
                                               (-1 + 7.594436687028424 * Z**2))
            )) / (3.588495042139688 + 7.594436687028424 * Z**2)
    def UU(Z):
        return (0.20405148639750417 * (
                1.1778865025892307 * Z
                + ttilde_e * 10.102197432038 * np.sqrt((1 - 0.5721009760467486 * Z**2) *
                                            (-1 + 7.594436687028424 * Z**2))
            )) / (3.6622434357916323 - 2.099237700754583 * Z**2)
