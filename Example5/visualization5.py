# Interactive Visualization for "Quasi-Symmetric Nets: A Constructive Approach to the Equimodular Elliptic Type of Kokotsakis Polyhedra" (Example 5)
...
def find_quadratic_roots(alpha, beta, gamma, delta, given_var, given_var_position):
    """
    Solves a quadratic equation derived from the provided angles and a given variable.

    This function computes the roots of a specific quadratic equation based on the given
    angles (alpha, beta, gamma, delta) and a known variable (x or y), depending on the
    specified position. It determines whether the roots are real and returns them along
    with a success flag.

    Parameters:
        alpha (float): An angle in degrees.
        beta (float): An angle in degrees.
        gamma (float): An angle in degrees.
        delta (float): An angle in degrees.
        given_var (float): The known variable value (x or y), depending on `given_var_position`.
        given_var_position (int): Indicates which variable is given:
            - 1: x is given, solve for y.
            - 2: y is given, solve for x.
            - 3: Solve with x = 0.
            - 4: Solve with y = 0.

    Returns:
        tuple:
            - roots (tuple): A tuple containing the computed roots (floats).
            - success (bool): True if real roots are found, False otherwise.

    Raises:
        ValueError: If `given_var_position` is not one of 1, 2, 3, or 4.
    """
    # Ensure inputs are floats for consistency and precision
    alpha = np.float64(alpha)
    beta = np.float64(beta)
    gamma = np.float64(gamma)
    delta = np.float64(delta)
    given_var = np.float64(given_var)
    
    # Compute sigma as half the sum of the angles
    sigma = (alpha + beta + gamma + delta) / np.float64(2.0)

    # Calculate coefficients using trigonometric functions (angles are in degrees)
    c22 = sin(sigma) * sin(sigma - beta)
    c20 = sin(sigma - gamma) * sin(sigma - beta - gamma)
    c02 = sin(sigma - alpha) * sin(sigma - beta - alpha)
    c11 = -sin(alpha) * sin(gamma)
    c00 = sin(sigma - delta) * sin(sigma - beta - delta)
    
    # Initialize quadratic coefficients a, b, c based on the given variable position
    if given_var_position == 1:
        # x is given, solve for y
        x = given_var
        a = c22 * x**2 + c02
        b = np.float64(2.0) * c11 * x
        c = c20 * x**2 + c00
    elif given_var_position == 2:
        # y is given, solve for x
        y = given_var
        a = c22 * y**2 + c20
        b = np.float64(2.0) * c11 * y
        c = c02 * y**2 + c00
    elif given_var_position == 3:
        # Solve c22 * y^2 + c20 = 0
        a = c22
        b = np.float64(0.0)
        c = c20
    elif given_var_position == 4:
        # Solve c22 * x^2 + c02 = 0
        a = c22
        b = np.float64(0.0)        
        c = c02
    else:
        raise ValueError("given_var_position must be one of 1 (x given), 2 (y given), 3, or 4")
    
    # Handle cases where the quadratic equation reduces to linear or constant
    if a == 0.0:
        if b == 0.0:
            if c == 0.0:
                # All coefficients are zero: infinite solutions exist
                return ("Infinite solutions exist.", False)
            else:
    ...
    # Compute the discriminant to check for real roots
    discriminant = b**2 - np.float64(4.0) * a * c

    # Check if the discriminant is negative, indicating complex roots
    if discriminant >= 0:
        sqrt_discriminant = np.sqrt(discriminant, dtype='float64')
        success = True
    else:
        # Discriminant is negative: complex roots encountered
        print("Complex roots encountered: Please choose another angle.")
        success = False
        return (None, None), success

    # Calculate the two real roots of the quadratic equation
    denominator = np.float64(2.0) * a
    root1 = (-b + sqrt_discriminant) / denominator
    root2 = (-b - sqrt_discriminant) / denominator
    
    return (root1, root2), success
...
def convert_angles_to_vertices(Alphas, Gammas, Deltas, DihedralAngles, Edge):
    ...
    a2 = generate_edge(a1, delta1, delta2, delta3, booster=0.15)  # Edge length A2A3
    ...
    A2B2 = generate_edge(a1, alpha1, alpha2, alpha3_tilde, booster=13) 
    ...
    A2C2 = generate_edge(a2, gamma3, gamma2, gamma1_tilde, booster=1)
    ...
    A3B3 = generate_edge(a4, alpha4, alpha3, alpha1_tilde, booster=1)
    ...
    gamma2_tilde = generate_tilde_angle(gamma1, gamma4, booster=0.1)  # Angle A4C4C1
    gamma3_tilde = 360.0 - gamma1 - gamma4 - gamma2_tilde  # Angle A1C1C4
    

    # Edge lengths for Plane 4
    A4C4 = generate_edge(a3, gamma1, gamma4, gamma2_tilde, booster=0.7)
        ...
        if phi != 0 and phi != 360:
            ...
            try:
                with np.errstate(invalid='raise', divide='raise'):
                    W2 = (0.8842187622039149 * (
                0.8494336559689466 * Z
                - tilde_e * 1.1387226496890441 * np.sqrt(
                    (1 + 3.4575776313801847 * Z**2) *
                    (1 - 0.7811714739558353 * Z**2)
                )
            )) / (-1.1465569838598522 - 3.4575776313801847 * Z**2)

                    W1 = (1.8303883744906646 * (
                0.739190870110122 * Z
                - tilde_e * 0.8185802872931142 * np.sqrt(
                    (1 + 3.4575776313801847 * Z**2) *
                    (1 - 0.7811714739558353 * Z**2)
                )
            )) / (-1.2264950862699229 - 3.4575776313801847 * Z**2)
                    
                    U = (0.18029302872898165 * (
                11.610011024543208 * Z
                + tilde_e * 6.663793331850769 * np.sqrt(
                    (1 + 3.4575776313801847 * Z**2) *
                    (1 - 0.7811714739558353 * Z**2)
                )
            )) / (2.6091661366212175 + 3.845171738795376 * Z**2)
...
def create_obj_file_of_vertices_and_faces(vertices):
    ...
    try:
        with open("VerticesFaces.obj", "w") as obj_file:
            obj_file.write(obj_text)
        print("OBJ file 'VerticesFaces.obj' created successfully.")
    except IOError as e:
...
def read_values_from_file(file_path):
    """
    Reads the values of Alphas, Betas, Gammas, Deltas, and DihedralAngles from a specified text file.

    The function expects the input file to contain lines in the following format:
        x_values = [...]
        Alphas = [...]
        Betas = [...]
        Gammas = [...]
    ...
    Each line should start with the variable name followed by an equals sign and a list of numerical values.

    Parameters:
        file_path (str): Path to the input file containing the angle and dihedral angle data.

    Returns:
        dict: A dictionary containing the following keys:
            - 'x_values': List of float64 values.
            - 'Alphas': List of float64 values.
            - 'Betas': List of float64 values.
            - 'Gammas': List of float64 values.
            - 'Deltas': List of float64 values.
            - 'DihedralAngles': List of float64 values.

    Raises:
        FileNotFoundError: If the specified file does not exist.
        ValueError: If there is an error parsing the data from the file.
        Exception: For any other exceptions that may occur during file reading or data parsing.

    Example:
        data = read_values_from_file('angles_data.txt')
        Alphas = data['Alphas']
        Betas = data['Betas']
        # and so on...

    Notes:
        - Ensure that the input file is correctly formatted to avoid parsing errors.
        - All numerical values are converted to NumPy float64 for precision.
    """
    data = {}
        ...
        with open(file_path, 'r') as file:
            for line in file:
                ...
                if '=' in line:
                    ...
                    # Use ast.literal_eval to safely parse the list of values
                    try:
                        value_list = ast.literal_eval(value_str)
                        # Convert all numerical values to np.float64 for precision
                        value_list = [np.float64(val) for val in value_list]
                        data[key] = value_list
                    except (SyntaxError, ValueError) as e:
                        raise ValueError(f"Error parsing values for '{key}': {e}")
                else:
                    print(f"Ignoring invalid line (no '='): {line}")

        # Check if all required keys are present
        required_keys = ['x_values', 'Alphas', 'Betas', 'Gammas', 'Deltas', 'DihedralAngles']
...
def calculate_tilde_e():
    eps1, eps2, eps3, eps4 = Epsilons
    e1, e2, e3 = Es[1:]
    
    Tilde_e_1 = 1 if (r2 - 1) * (r3 - 1) > 0 else -1
    Tilde_e_2 = 1 if (s2 - 1) * (1 - M) > 0 else -1
    
    theta1, theta2, theta3, theta4 = DihedralAngles_Init
    Z = 1 / tan(theta1 / 2)
    
    for Tilde_e in [-1, 1]:
        W2 = eps2 * np.sqrt(f2 * (f2 - M) * (r2 - 1) * (s2 - 1) / (1 - M)) * (Z + Tilde_e * e1 * np.sqrt((f2 - 1) * (Z**2 + 1 - r1) * (r1 - 1 - (1 - M) * Z**2) / (f2 * (f2 - M) * (r2 - 1)))) / (r2 - 1 + (f2 - 1) * Z**2) 

        W1 = eps1 * np.sqrt(f1 * (f1 - M) * (r1 - 1) * (s1 - 1) / (1 - M)) * (Z - Tilde_e * np.sqrt((f1 - 1) * (Z**2 + 1 - r1) * (r1 - 1 - (1 - M) * Z**2) / (f1 * (f1 - M) * (r1 - 1)))) / (r1 - 1 + (f1 - 1) * Z**2) 
        
        Q1 = ((1 - M) * np.sqrt(f2 * f3) + Tilde_e_1 * e1 * e2 * np.sqrt((f2 - 1) * (f2 - M) * (f3 - 1) * (f3 - M))) * (np.sqrt(Tilde_e_1 * (f2 - M)*(f3 - M)) + e1 * e2 * np.sqrt(Tilde_e_1 * f2 * f3 * (f2 - 1) * (f3 - 1)))

        Q2 = (1 - M - (f2 - 1) * (f3 - 1)) * (e1 * np.sqrt(Tilde_e_1 * f3 * (f2 - 1) * (f3 - M)) + e2 * np.sqrt(Tilde_e_1 * f2 * (f2 - M) * (f3 - 1)))

        Q3 = (1 - M) * (e1 * np.sqrt(Tilde_e_1 * f3 * (f2 - 1) * (f3 - M)) + e2 * np.sqrt(Tilde_e_1 * f2 * (f2 - M) * (f3 - 1)))**2


        U = Tilde_e_2 * eps2 * eps3 * np.sqrt(Tilde_e_1 * (r2 - 1) * (r3 - 1)) * (Q1 * Z + Tilde_e * Q2 * np.sqrt((Z**2 + 1 - r2) * (r2 - 1 - (1 - M) * Z**2) / (r2 - 1))) / ((r2 - 1) * (1 - M - (f2 - 1) * (f3 - 1))**2 + Tilde_e_1 * Q3 * Z**2)
        
        if np.isclose(W2, 1 / tan(theta2 / 2), atol=1e-6) and np.isclose(U, 1 / tan(theta3 / 2), atol=1e-6) and np.isclose(W1, 1 / tan(theta4 / 2), atol=1e-6):
            print("YES")
            return Tilde_e
        else:
            print(f"W2 = {W2}, U = {U}, W1 = {W1}")
            print(f"1/tan(theta2 / 2) = {1 / tan(theta2 / 2)}, 1/tan(theta3 / 2) = {1 / tan(theta3 / 2)}, 1/tan(theta4 / 2) = {1 / tan(theta4 / 2)}")
            print(f"diff = {np.abs(W2 - 1 / tan(theta2 / 2))}, {np.abs(U - 1 / tan(theta3 / 2))}, {np.abs(W1 - 1 / tan(theta4 / 2))}")
            print("NO")
            continue
...
def main():
    ...
    Alphas = [26.20863403213998, 16.166237389600262, 134.65533802039442, 117.95117201340666]
    Betas = [82.2407675648952, 130.87095233025335, 34.44439013740831, 49.52829397349284]
    Gammas = [21.949109994264898, 18.85247535405415, 145.3694664686027, 149.0275482144225]
    Deltas = [60, 115, 80, 105]
    ...
    def WW1(Z):
        return (1.8303883744906646 * (
                0.739190870110122 * Z
                - ttilde_e * 0.8185802872931142 * np.sqrt(
                    (1 + 3.4575776313801847 * Z**2) *
                    (1 - 0.7811714739558353 * Z**2)
                )
            )) / (-1.2264950862699229 - 3.4575776313801847 * Z**2)
    ...
    def WW2(Z):
        return (0.8842187622039149 * (
                0.8494336559689466 * Z
                - ttilde_e * 1.1387226496890441 * np.sqrt(
                    (1 + 3.4575776313801847 * Z**2) *
                    (1 - 0.7811714739558353 * Z**2)
                )
            )) / (-1.1465569838598522 - 3.4575776313801847 * Z**2)
    ...
    def UU(Z):
        return (0.18029302872898165 * (
                11.610011024543208 * Z
                + ttilde_e * 6.663793331850769 * np.sqrt(
                    (1 + 3.4575776313801847 * Z**2) *
                    (1 - 0.7811714739558353 * Z**2)
                )
            )) / (2.6091661366212175 + 3.845171738795376 * Z**2)
    ...
    DihedralAngles = [120, 140, 110, 130]
