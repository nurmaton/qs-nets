# Interactive Visualization for "Quasi-Symmetric Nets: A Constructive Approach to the Equimodular Elliptic Type of Kokotsakis Polyhedra" (Example 3)
...
def convert_angles_to_vertices(Alphas, Gammas, Deltas, DihedralAngles, Edge):
    ...
    A2C2 = generate_edge(a2, gamma3, gamma2, gamma1_tilde, booster=0.6)
    ...
    A3B3 = generate_edge(a4, alpha4, alpha3, alpha1_tilde, booster=0.9)
    ...
    A4C4 = generate_edge(a3, gamma1, gamma4, gamma2_tilde, booster=2)
        ...
        if phi != 0 and phi != 360:
            ...
            try:
                with np.errstate(invalid='raise', divide='raise'):
                    W2 = (5 * np.sqrt(7) * Z - tilde_e * np.sqrt(10 * (2 - 3 * Z**2) * (3 * Z**2 - 1))) / (4 + 9 * Z**2)

                    W1 = (6 * Z - tilde_e * np.sqrt(2 * (2 - 3 * Z**2) * (3 * Z**2 - 1))) / (1 + 3 * Z**2)
...
def main():
    ...
    ttilde_e = 1
    def WW1(Z):
        return (6 * Z - ttilde_e * np.sqrt(2 * (3 * Z**2 - 1) * (2 - 3 * Z**2))) / (1 + 3 * Z**2)

    def WW2(Z):
        return (5 * np.sqrt(7) * Z - ttilde_e * np.sqrt(10 * (3 * Z**2 - 1) * (2 - 3 * Z**2))) / (4 + 9 * Z**2)

    def UU(Z):
        return 1 / Z
    
    ZZ = 3/5
    # ZZ = np.sqrt(2) / np.sqrt(3)
    WW1_vals = WW1(ZZ)
    WW2_vals = WW2(ZZ)
    UU_vals = UU(ZZ)
    print(WW1_vals)
